...
public class Grammar {
    ...
    public String classify() {
        ...
        for (char lhs : P.keySet()) {
            for (String rhs : P.get(lhs)) {
                if (!(rhs.length() == 1 && VT.contains(rhs.charAt(0))) &&
                        !(rhs.length() == 2 && ((VT.contains(rhs.charAt(0)) && VN.contains(rhs.charAt(1))) ||
                                (VN.contains(rhs.charAt(0)) && VT.contains(rhs.charAt(1)))))) {
                    isRegular = false;
                }
                if (!VN.contains(lhs)) {
                    isContextFree = false;
